...
#include "../inc/dialogue.h" // For printing location descriptions and plot dialogue
#include "../inc/battle.h" // Needed to potentially trigger battles on arrival
#include "../inc/kingsguard.h" // Needed for battle setup
#include "../inc/puzzle.h" // Needed to trigger puzzle on arrival
...
int get_location_id(const char *name) {
    ...
    return LOCATION_UNKNOWN; // Not found
}
...
void look_around(void)
{
    ...
    // Print description based on location ID
    switch (current_loc_id) {
        ...
        case LOCATION_COURTYARD:
            ...
            if (!game.plot_flags.heir_battle_fought && game.plot_flags.met_initial_kingsguard) {
                 ...
                 // Trigger Heir Battle
                 Kingsguard *opponents[] = {get_kingsguard_by_name("Heir_Guard")}; // Get template pointer
                 start_battle(opponents, 1);
                 // Note: start_battle changes game state to STATE_BATTLE
                 game.plot_flags.heir_battle_fought = true; // Set flag so it doesn't trigger again
            }
            break;
        case LOCATION_SUMMONING_HALL:
             ...
             if (!game.plot_flags.saber_recruited && game.plot_flags.has_summoning_artifact) {
                  ...
                  start_saber_puzzle(); // Changes state to STATE_PUZZLE
             }
            break;
        case LOCATION_COLOSSEUM:
            print_dialogue("look_colosseum");
             ...
             if (!game.plot_flags.final_battle_intro_shown && game.plot_flags.saber_recruited) { // Requires Saber recruited
                 ...
                 }; // Get template pointers
                 start_battle(opponents, 3);
                 game.plot_flags.final_battle_intro_shown = true; // Set flag
                 // Note: start_battle changes game state to STATE_BATTLE
             }
            break;
        default:
...
// Removed the old go_to_location function from here.
// The movement logic is now in player_go_to_location in game_state.c
